...
def course_recommender(course_list):
    ...
    c = 0
    rec_course = []
    no_of_reco = st.slider('Choose Number of Course Recommendations:', 1, 4, 4)
...
user_data_columns = ['Name', 'Email', 'Resume Score', 'Timestamp', 'Total Page', 'Predicted Field',
                     'User Level', 'Actual Skills', 'Recommended Skills', 'Recommended Course']
user_data_df = pd.DataFrame(columns=user_data_columns)
def insert_data(name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills,
                courses):
    global user_data_df
    new_data = pd.DataFrame([[name, email, str(res_score), timestamp, str(no_of_pages), reco_field, cand_level,
                              str(skills), str(recommended_skills), str(courses)]], columns=user_data_columns)
    user_data_df = user_data_df.append(new_data, ignore_index=True)
